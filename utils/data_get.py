...
class variflight(object):
    ...
    def get_mesbyurl(self, url):
        ...
        if mylist:
            for selector in mylist:
                ...
                get = self.get_detail_mes(detail_url)

                mydict = {
                    "flight_id": airline[1],                    # 航班号
                    "company":airline[0],                       # 航空公司
                    
                    "plan_departure_time": plan_up[0],          # 计划起飞
                    "plan_arrival_time": plan_down[0],          # 计划到达
                    
                    "actual_departure_time": real_up,           # 实际起飞
                    "actual_arrival_time": real_down,           # 实际到达

                    "flight_status": state[0],                  # 航班状态
                    
                    "departure": depart[0],                     # 出发地
                    "arrival": arrive[0],                       # 到达地
                    
                    "punctuality_rate": get[0],                 # 准点率
                    ...
                    #"detail_url":self.base_url+detail_url[0]   # 获取详细信息url
                }
    ...
    def get_detail_mes(self,url):
        ...
        if mylist:
            ...
            plan_up = plan_up[0] + ' ' + plan_up[1]
            plan_down = plan_down[0] + ' ' + plan_down[1]
            ...
            mydict = {
                ...
                'plan_departure_time':plan_up,              # 计划起飞时间
                'plan_arrival_time':plan_down,              # 计划到达时间
                "actual_departure_time": depart_t,          # 实际起飞或预计起飞时间
                "actual_arrival_time": arrive_t,            # 实际到达或预计到达时间
            ...
            '''
            result = [ac,self.check(forecast),depart_s,depart_e,arrive_e,arrive_s]
        else:
            print("航班不存在信息")
...
def main():
    ...
    '''
    print(json.dumps(vf.search_num('CA1553'),indent=2,ensure_ascii=False))
    print(json.dumps(vf.search_seg('北京','包头'),indent=2,ensure_ascii=False))
    print(json.dumps(vf.get_detail_mes('http://www.variflight.com/schedule/PEK-BAV-CA1107.html?AE71649A58c77=')[0],indent=2,ensure_ascii=False))
    '''
